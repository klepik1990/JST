...
module.exports = {
    ...
    async createFolder(folderName, subfolderName = undefined) {
        if (subfolderName === undefined) {
            ...
            if (response.statusCode === 409) {
                let deleteResponse = await unirest.delete(`${this.elements.URL}resources?path=${folderName}&permanently=true`)
                    .header(this.elements.customHeaders);
    ...
    async fileUpload(folderName, subfolderName = undefined, fileName) {
        let newFile;
        assert.notEqual(fileName, undefined, 'Не введено имя файла.');
        if (subfolderName !== undefined) {
            newFile = await unirest.get(`${this.elements.URL}resources/upload?path=${folderName}/${subfolderName}/${fileName}.jpeg&overwrite=true`).header(this.elements.customHeaders);
            await unirest.put(newFile.body.href);
        } else {
            newFile = await unirest.get(`${this.elements.URL}resources/upload?path=${folderName}/${fileName}.jpeg&overwrite=true`).header(this.elements.customHeaders);
            await unirest.put(newFile.body.href);
        }
    },
    ...
    async createFiles(folderName, fileName1, fileName2) {
        ...
        let file1 = await unirest.post(`${this.elements.URL}resources/upload?url=${this.elements.image1}&path=${folderName}/${fileName1}.jpeg`)
            .header(this.elements.customHeaders);
        let file2 = await unirest.post(`${this.elements.URL}resources/upload?url=${this.elements.image2}&path=${folderName}/${fileName2}.jpeg`)
            .header(this.elements.customHeaders);
        setTimeout(async () => {
            await unirest.get(`${file1.body.href}`).header(this.elements.customHeaders).end((response) => {console.log('Статус загрузки файла:', response.body.status);
            assert.equal(response.body.status, 'success', 'Статус не success.');});
            await unirest.get(`${file2.body.href}`).header(this.elements.customHeaders).end((response) => {console.log('Статус загрузки файла:', response.body.status);
            assert.equal(response.body.status, 'success', 'Статус не success.');});
        }, 2000);
    },

    async deleteNonEmptyFolder(folderName) {
        let folder = await unirest.delete(`${this.elements.URL}resources?path=${folderName}&permanently=true`).header(this.elements.customHeaders);
        setTimeout(async () => {
            await unirest.get(`${folder.body.href}`).header(this.elements.customHeaders).end((response) => {console.log('Статус удаления папки с файлами:', response.body.status);
            assert.equal(response.body.status, 'success', 'Статус удаления не success');});
        }, 3000);
    },

    async clearBucket() {
        let bucketResponse = await unirest.delete(`${this.elements.URL}trash/resources`).header(this.elements.customHeaders);
        setTimeout(async () => {
            await unirest.get(`${bucketResponse.body.href}`).header(this.elements.customHeaders).end((response) => {console.log('Статус очистки корзины:', response.body.status);
